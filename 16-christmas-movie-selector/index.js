...
function selectedMovie() {
  for (let i = 0; i < moviesArr.length; i++) {
    if (
      ageSelector.value === moviesArr[i].age &&
      genreSelector.value === moviesArr[i].genre
    ) {
      suggestedMovie.innerHTML = moviesArr[i].name;
    }
  }
}

btn.addEventListener("click", selectedMovie);
